...
# 初始化函数，设置数据集的路径、转换、标签等属性
    def __init__(self, root_dir, transform=None):
        ...
        self.labels = np.array([0, 1]) # 数据集的标签列表 #1=positive, 0=negative
  ...
  # 根据给定的索引返回一个样本（实例集合）及其标签
    def __getitem__(self, idx):
        ...
        label = self.labels[1] if "positive" in bag_name else self.labels[0] # 根据样本文件夹名判断标签，假设包含"positive"或"negative"字样
