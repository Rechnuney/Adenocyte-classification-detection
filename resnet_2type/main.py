...
perf_path = '/remote-home/chenyuren/resnet_2type/performance.txt'
...
if __name__ == '__main__':
    ...
    weight = torch.tensor([1, 12984/1912]).cuda()
    ...
    trainset = MyDataset(phase='train', data_path='/remote-home/share/DATA/RedHouse/AdenocyteBag/bags', transform=transform)
    testset = MyDataset(phase='test', data_path='/remote-home/share/DATA/RedHouse/AdenocyteBag/bags', transform=transform)
    #valset = MyDataset(phase='val', data_path='/remote-home/share/DATA/RedHouse/AdenocyteBag/bags', transform=transform)
    ...
    model = ResNet_s(BasicBlock, [2,2,2,2], num_classes=2)
    if torch.cuda.is_available():
        model = model.cuda()
    ...
    for epoch in range(epochs_num):
        ...
        _, val_acc, real_labels, predicted_labels = validate(test_dl, model, criteria)
        ...
        print('Val:\t Best accuracy: {:.4f}, Best epoch: {}\n'.format(best_acc, best_epoch))

        f = open(perf_path, 'a+')
        f.write("confusion matrix = \n")
        f.write(str(metrics.confusion_matrix(real_labels, predicted_labels)))
        f.write('Val:\t Best accuracy: {:.4f}, Best epoch: {}\n'.format(best_acc, best_epoch))
        f.write('\n')
        f.close()
